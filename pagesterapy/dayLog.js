...
for (let i = 2; i < datesList.length - 1; i++) {
  if (datesList[i].getDay() === 0 || datesList[i].getDay() === 6 || datesList[i].getDay() === 1) {
...
secondDatesList.push(datesList[datesList.length - 1]); // Add the last element
  ...
  secondDatesList.forEach((date) => {
    ...
    if (date.getDay() === 0 || date.getDay() === 6) {
      ...
      if(date.getTime()<secondDatesList[5].getTime()&&(datesList[0].getDay() === 5||datesList[0].getDay() ===6)){withZavVidWekkend = true;}
      if(date === datesList[10]||date === datesList[20]||date === datesList[30]){
        etapnyjEpicris = true;
      }
